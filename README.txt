ThinReports Editor
<http://www.thinreports.org/>

Copyright (C) 2010-2011 Matsukei Co.,Ltd. All rights reserved.
This tool is licensed under a GNU GENERAL PUBLIC LICENSE
Version 3 license <http://www.gnu.org/licenses/gpl.html>.

- Documentations
  
  Changelog
    <http://osc.matsukei.net/projects/thinreports/wiki/Changelog>
  Getting Started
    <http://osc.matsukei.net/projects/thinreports/wiki/Getting_Started>
  Examples
    <http://osc.matsukei.net/projects/thinreports/wiki/Examples>
  Howtos
    <http://osc.matsukei.net/projects/thinreports/wiki/HowTos>

- Help

  Open Forum
    <http://osc.matsukei.net/projects/thinreports/boards>

- News and Social

  Project News
    <http://osc.matsukei.net/projects/thinreports/news>
  Twitter (@thinreports_org)
    <http://twitter.com/#!/thinreports_org>
  facebook
    <http://www.facebook.com/ThinReports.org>

- Project

  Matsukei Open Source Center
    <http://osc.matsukei.net/projects/thinreports>

- Source Code
  
  Matsukei Open Source Center
    <http://osc.matsukei.net/projects/thinreports-editor/repository>
    <http://osc.matsukei.net/projects/thinreports-generator/repository>
  GitHub (Mirror)
    <https://github.com/thinreports>
  
--------------------------------------------------------------------------------

- IPA Fonts

  Copyright(c) 2003-2011 Information-technology Promotion Agency, Japan.
  Please see the lincense <http://ipafont.ipa.go.jp/ipa_font_license_v1.html>.

- Fugue Icons

  Copyright (C) 2010 Yusuke Kamiyamane. All rights reserved.
  <http://p.yusukekamiyamane.com/>

  The following are stored in assets/icons/*:
   - arrow-curve.png
   - arrow-curve-180-left.png
   - balloons-white.png
   - color-swatch.png
   - cross-button.png
   - disk-black.png
   - disks-black.png
   - document-export.png
   - document--gear.png
   - document-medium.png
   - document--plus.png
   - edit-alignment.png
   - edit-alignment-center.png
   - edit-alignment-right.png
   - edit-bold.png
   - edit-italic.png
   - edit-strike.png
   - edit-underline.png
   - edit-vertical-alignment.png
   - edit-vertical-alignment-middle.png
   - edit-vertical-alignment-top.png
   - exclamation-octagon.png
   - exclamation-red.png
   - folder-horizontal-open.png
   - guide.png
   - hoge.txt
   - information.png
   - layer-resize-actual.png
   - layer-resize-replicate.png
   - layer-resize-replicate-vertical.png
   - layers-alignment.png
   - layers-alignment-bottom.png
   - layers-alignment-center.png
   - layers-alignment-left.png
   - layers-alignment-middle.png
   - layers-alignment-right.png
   - layers-arrange.png
   - layers-arrange-back.png
   - layers-stack-arrange.png
   - layers-stack-arrange-back.png
   - magnifier-zoom-in.png
   - magnifier-zoom-out.png
   - plus-button.png
   - question-white.png
   - selection-input.png
   - toggle-small.png
   - toggle-small-expand.png
   - zoom-in.png
   - zoom-out.png